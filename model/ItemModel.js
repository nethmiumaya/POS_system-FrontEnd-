export function saveItem(item, callback) {
  ...
  http.open("POST", "http://localhost:8080/app/item", true);
...
export function getAllItems() {
  return new Promise((resolve, reject) => {
    ...
    http.open("GET", "http://localhost:8080/app/item", true);
    http.send();
  });
}
...
export function updateItem(item) {
  ...
  http.open("PUT", "http://localhost:8080/app/item", true);
  http.setRequestHeader("Content-Type", "application/json");
  http.send(itemJSON);
}
...
export function deleteItem(itemId) {
  ...
  http.open("DELETE",`http://localhost:8080/app/item?itemId=${itemId}`,
     true
    );
  http.send();
}
